...
namespace Universe.CQRS.Dal.Queries
{
        ...
        where TDbEntityRequest : BaseDbEntityRequest, IDbEntityRequest
    {
            ...
            bool disablePropsMiSearch = false)
        {
            ...
            var mi = DbQueryExtensions.CreateDbRequestMetaInfo<TEntityDb, TDbEntityRequest>(query, container?.FieldMap, disablePropsMiSearch);

            // Результат работы запроса
            var projectionResult = await PagingExtensions.GetCurrentPageExtensionAsync<TDbEntityRequest>(query
                    .ApplyFiltersAtQuery(req.Filters, mi, req.AllowNoTrackingMode)
                    .ApplySortingAtQuery(req.Sorting, mi), req.Paging);
            ...
            bool disablePropsMiSearch = false)
        {
            ...
            // Результат работы запроса
            var projectionResult = PagingExtensions.GetCurrentPageExtension<TDbEntityRequest>(query
                    .ApplyFiltersAtQuery(req.Filters, mi, req.AllowNoTrackingMode)
                    .ApplySortingAtQuery(req.Sorting, mi), req.Paging);
