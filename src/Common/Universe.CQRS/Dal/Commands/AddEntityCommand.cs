...
namespace Universe.CQRS.Dal.Commands
{
    ...
    public class AddEntityCommand<TEntityDb> : BaseCommand
        where TEntityDb : Entity 
    {
        protected TEntityDb CreatedEntity { get; set; }

        public virtual async Task<AddEntityResult> ExecuteAsync(TEntityDb entityDb)
        {
            if (entityDb == null)
        ...
        public virtual AddEntityResult Execute(TEntityDb entityDb)
        {
            if (entityDb == null)
                throw new ArgumentNullException(nameof(entityDb));
