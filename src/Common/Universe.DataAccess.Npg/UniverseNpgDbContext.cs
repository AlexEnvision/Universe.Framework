...
namespace Universe.DataAccess.Npg
{
    ...
    public class UniverseNpgDbContext : DbContext, IUniverseDbContext
    {
        ...
        {
            if (!connectionString.IsNullOrEmpty() && connectionString.Contains("server"))
            {
        ...
        where TConfiguration : DbMigrationsConfiguration<TUniverseDbContext>, new()
    {
        ...
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            ...
            var maps = Enumerable.Cast<IEntityMap>(Enumerable.Select(Enumerable.Where(typeof(TUniverseDbContext).Assembly
                    .GetTypes(), x => !x.IsAbstract && Enumerable.Contains(x.GetInterfaces(), typeof(IEntityMap))), Activator.CreateInstance));
