...
namespace Universe.NumbesTypes
{
    ...
    public struct Quaduple : IComparable<Quaduple> //Quadruple
    {
        #region Public constants
        /// <summary>
        /// 0.  Equivalent to (Quad)0.
        /// </summary>
        public static readonly Quaduple Zero = new Quaduple(0UL, long.MinValue); //there is only one zero; all other significands with exponent long.MinValue are invalid.

        /// <summary>
        /// 1.  Equivalent to (Quad)1.
        /// </summary>
        public static readonly Quaduple One = (Quaduple)1UL; //used for increment/decrement operators

        /// <summary>
        /// Positive infinity.  Equivalent to (Quad)double.PositiveInfinity.
        /// </summary>
        public static readonly Quaduple PositiveInfinity = new Quaduple(0UL, infinityExponent);

        /// <summary>
        /// Negative infinity.  Equivalent to (Quad)double.NegativeInfinity.
        /// </summary>
        public static readonly Quaduple NegativeInfinity = new Quaduple(0UL, negativeInfinityExponent);

        /// <summary>
        /// The Not-A-Number value.  Equivalent to (Quad)double.NaN.
        /// </summary>
        public static readonly Quaduple NaN = new Quaduple(0UL, notANumberExponent);

        /// <summary>
        /// The maximum value representable by a Quad, (2 - 1/(2^63)) * 2^(long.MaxValue-65)
        /// </summary>
        public static readonly Quaduple MaxValue = new Quaduple(~highestBit, exponentUpperBound);

        /// <summary>
        /// The minimum value representable by a Quad, -(2 - 1/(2^63)) * 2^(long.MaxValue-65)
        /// </summary>
        public static readonly Quaduple MinValue = new Quaduple(ulong.MaxValue, exponentUpperBound);

        /// <summary>
        /// The smallest positive value greater than zero representable by a Quad, 2^(long.MinValue+65)
        /// </summary>
        public static readonly Quaduple Epsilon = new Quaduple(0UL, exponentLowerBound);

        /// <summary>
        /// Представляет основание натурального логарифма, определяемое константой e.
        /// </summary>
        public static readonly Quaduple E = 2.718281828459045235360287471352662497757247093699959574966967627724076630353547594571382178525166427427466391932003059921817413596629043572900334295260595630738132328627943490763233; //8298807531 9525101901 1573834187 9307021540 8914993488 4167509244 7614606680 8226480016 8477411853 7423454424 3710753907 7744992069 5517027618 3860626133 1384583000 7520449338 2656029760 6737113200 7093287091 2744374704 7230696977 2093101416 9283681902 5515108657 4637721112 5238978442 5056953696 7707854499 6996794686 4454905987 9316368892 3009879312 7736178215 4249992295 7635148220 8269895193 6680331825 2886939849 6465105820 9392398294 8879332036 2509443117 3012381970 6841614039 7019837679 3206832823 7646480429 5311802328 7825098194 5581530175 6717361332 0698112509 9618188159 3041690351 5988885193 4580727386 6738589422 8792284998 9208680582 5749279610 4841984443 6346324496 8487560233 6248270419 7862320900 2160990235 3043699418 4914631409 3431738143 6405462531 5209618369 0888707016 7683964243 7814059271 4563549061 3031072085 1038375051 0115747704 1718986106 8739696552 1267154688 9570350354;
        #endregion
        ...
        public Quaduple(ulong significandBits, long exponent)
        {
        ...
        /// <param name="exponent"></param>
        public Quaduple(long significand, long exponent)
        {
        ...
        private static readonly Quaduple[,] specialDivisionTable = new Quaduple[6, 6]{
        ...
        private static readonly Quaduple[,] specialMultiplicationTable = new Quaduple[6, 6]{
        ...
        private static readonly Quaduple[,] specialSubtractionTable = new Quaduple[6, 6]{
        ...
        private static readonly Quaduple[,] specialAdditionTable = new Quaduple[6, 6]{
        ...
        private static readonly Quaduple e19 = (Quaduple)10000000000000000000UL;
        private static readonly Quaduple e10 = (Quaduple)10000000000UL;
        private static readonly Quaduple e5 = (Quaduple)100000UL;
        private static readonly Quaduple e3 = (Quaduple)1000UL;
        private static readonly Quaduple e1 = (Quaduple)10UL;

        private static readonly Quaduple en19 = One / e19;
        private static readonly Quaduple en10 = One / e10;
        private static readonly Quaduple en5 = One / e5;
        private static readonly Quaduple en3 = One / e3;
        private static readonly Quaduple en1 = One / e1;

        private static readonly Quaduple en18 = One / (Quaduple)1000000000000000000UL;
        private static readonly Quaduple en9 = One / (Quaduple)1000000000UL;
        private static readonly Quaduple en4 = One / (Quaduple)10000UL;
        private static readonly Quaduple en2 = One / (Quaduple)100UL;
        ...
        Parsed:
            ...
            {
                Quaduple result = specialMultiplicationTable[(int)(this.Exponent - zeroExponent), multiplier.Exponent > notANumberExponent ? (int)(4 + (multiplier.SignificandBits >> 63)) : (int)(multiplier.Exponent - zeroExponent)];
            ...
            {
                Quaduple result = specialMultiplicationTable[(int)(4 + (this.SignificandBits >> 63)), (int)(multiplier.Exponent - zeroExponent)];
                this.SignificandBits = result.SignificandBits;
                this.Exponent = result.Exponent;
                return;
            }
        ...
        public void Multiply(Quaduple multiplier)
        {
            if (this.Exponent <= notANumberExponent) //zero/infinity/NaN * something            
            {
                Quaduple result = specialMultiplicationTable[(int)(this.Exponent - zeroExponent), multiplier.Exponent > notANumberExponent ? (int)(4 + (multiplier.SignificandBits >> 63)) : (int)(multiplier.Exponent - zeroExponent)];
        ...
        public void MultiplyUnchecked(Quaduple multiplier)
        {
            if (this.Exponent <= notANumberExponent) //zero/infinity/NaN * something            
            {
                Quaduple result = specialMultiplicationTable[(int)(this.Exponent - zeroExponent), multiplier.Exponent > notANumberExponent ? (int)(4 + (multiplier.SignificandBits >> 63)) : (int)(multiplier.Exponent - zeroExponent)];
        ...
        Parsed:
            ...
            #region Addition
            {
                if (this.Exponent <= notANumberExponent) //zero or infinity or NaN + something
                {
                    ...
                    {
                        Quaduple result = specialAdditionTable[(int)(this.Exponent - zeroExponent), value.Exponent > notANumberExponent ? (int)(4 + (value.SignificandBits >> 63)) : (int)(value.Exponent - zeroExponent)];
                        this.SignificandBits = result.SignificandBits;
                        this.Exponent = result.Exponent;
                    }
                ...
                else if (value.Exponent <= notANumberExponent) //finite + (infinity or NaN)
                {
                    ...
                    {
                        Quaduple result = specialAdditionTable[(int)(4 + (this.SignificandBits >> 63)), (int)(value.Exponent - zeroExponent)];
                        this.SignificandBits = result.SignificandBits;
                        this.Exponent = result.Exponent;
                    }
                ...
                {
                    Subtract(new Quaduple(value.SignificandBits ^ highestBit, value.Exponent));
                    return;
                }
        ...
        public void Add(Quaduple value)
        {
            #region Addition
        ...
        Parsed:
            ...
            if (this.Exponent <= notANumberExponent) //infinity or NaN - something
            {
                ...
                {
                    Quaduple result = specialSubtractionTable[(int)(this.Exponent - zeroExponent), value.Exponent > notANumberExponent ? (int)(4 + (value.SignificandBits >> 63)) : (int)(value.Exponent - zeroExponent)];
                    this.SignificandBits = result.SignificandBits;
                    this.Exponent = result.Exponent;
                }
            ...
            else if (value.Exponent <= notANumberExponent) //finite - (infinity or NaN)
            {
                if (value.Exponent != zeroExponent)
                {
                    Quaduple result = specialSubtractionTable[(int)(4 + (this.SignificandBits >> 63)), (int)(value.Exponent - zeroExponent)];
                    this.SignificandBits = result.SignificandBits;
                    this.Exponent = result.Exponent;
                }
            ...
            {
                this.Add(new Quaduple(value.SignificandBits ^ highestBit, value.Exponent));
                return;
            }
        ...
        public void Subtract(Quaduple value)
        {
            #region Subtraction
            if (this.Exponent <= notANumberExponent) //infinity or NaN - something
            {
        ...
        Parsed:
            ...
            {
                Quaduple result = specialDivisionTable[(int)(this.Exponent - zeroExponent), divisor.Exponent > notANumberExponent ? (int)(4 + (divisor.SignificandBits >> 63)) : (int)(divisor.Exponent - zeroExponent)];
                this.SignificandBits = result.SignificandBits;
                this.Exponent = result.Exponent;
                return;
            }
            ...
            {
                Quaduple result = specialDivisionTable[(int)(4 + (this.SignificandBits >> 63)), (int)(divisor.Exponent - zeroExponent)];
                this.SignificandBits = result.SignificandBits;
                this.Exponent = result.Exponent;
                return;
            }
        ...
        public void Divide(Quaduple divisor)
        {
            #region Division
            if (this.Exponent <= notANumberExponent) //zero/infinity/NaN divided by something
            {
        ...
        /// <returns></returns>        
        public static Quaduple operator <<(Quaduple qd, int shift)
        {
            if (qd.Exponent <= notANumberExponent)
                return qd; //finite * infinity == infinity, finite * NaN == NaN, finite * 0 == 0
            else
                return new Quaduple(qd.SignificandBits, qd.Exponent + shift);
        }
        ...
        /// <param name="shift"></param>
        /// <returns></returns>        
        public static Quaduple LeftShift(Quaduple qd, long shift)
        {
            if (qd.Exponent <= notANumberExponent)
                return qd; //finite * infinity == infinity, finite * NaN == NaN, finite * 0 == 0
            else
        ...
        /// <param name="shift"></param>
        /// <returns></returns>
        public static Quaduple RightShift(Quaduple qd, long shift)
        {
            if (qd.Exponent <= notANumberExponent)
                return qd; //infinity / finite == infinity, NaN / finite == NaN, 0 / finite == 0
            else
        ...
        /// </remarks>
        public static Quaduple operator /(Quaduple qd1, Quaduple qd2)
        {
        ...
        public static Quaduple operator -(Quaduple qd)
        {
            if (qd.Exponent <= notANumberExponent)
            {
                ...
                return new Quaduple(qd.SignificandBits ^ highestBit, qd.Exponent); //just swap the sign bit                        
        }
        ...
        public static Quaduple operator +(Quaduple qd1, Quaduple qd2)
        {
            ...
            {
                return qd1 - new Quaduple(qd2.SignificandBits ^ highestBit, qd2.Exponent);
            }
            ...
            if (qd1.Exponent > qd2.Exponent)
            {
                ...
                else
                {
                    ...
                    if (bits < highestBit) //this can only happen in an overflow                    
                        result = new Quaduple((qd1.SignificandBits & highestBit) | (bits >> 1), qd1.Exponent + 1);
                    else
                        return new Quaduple((qd1.SignificandBits & highestBit) | (bits & ~highestBit), qd1.Exponent);
                }
            }
            else if (qd1.Exponent < qd2.Exponent)
            {
                ...
                else
                {
                    ...
                    if (bits < highestBit) //this can only happen in an overflow                    
                        result = new Quaduple((qd2.SignificandBits & highestBit) | (bits >> 1), qd2.Exponent + 1);
                    else
                        return new Quaduple((qd2.SignificandBits & highestBit) | (bits & ~highestBit), qd2.Exponent);
                }
            }
            else //expDiff == 0
            {
                //the MSB must have the same sign, so the MSB will become 0, and logical overflow is guaranteed in this situation (so we can shift right and increment the exponent).
                result = new Quaduple(((qd1.SignificandBits + qd2.SignificandBits) >> 1) | (qd1.SignificandBits & highestBit), qd1.Exponent + 1);
            }
        ...
        public static Quaduple operator -(Quaduple qd1, Quaduple qd2)
        {
            ...
            if (qd1.Exponent > qd2.Exponent)
            {
                ...
                else
                {
                    ...
                    //make sure MSB is 1                       
                    int highestBitPos = nlz(bits);
                    result = new Quaduple(((bits << highestBitPos) & ~highestBit) | (qd1.SignificandBits & highestBit), qd1.Exponent - highestBitPos);
                }
            }
            else if (qd1.Exponent < qd2.Exponent) //must subtract qd1's significand from qd2, and switch the sign
            {
                if (qd2.Exponent >= qd1.Exponent + 64)
                    return new Quaduple(qd2.SignificandBits ^ highestBit, qd2.Exponent); //qd1 too small to matter, switch sign of qd2 and return

                ulong bits = (qd2.SignificandBits | highestBit) - ((qd1.SignificandBits | highestBit) >> (int)(qd2.Exponent - qd1.Exponent));

                //make sure MSB is 1                       
                int highestBitPos = nlz(bits);
                result = new Quaduple(((bits << highestBitPos) & ~highestBit) | (~qd2.SignificandBits & highestBit), qd2.Exponent - highestBitPos);
            }
        ...
        public static Quaduple operator *(Quaduple qd1, Quaduple qd2)
        {
            ...
            if (significandBits < (1UL << 63))
            {
                ...
                result = new Quaduple(((qd1.SignificandBits ^ qd2.SignificandBits) & highestBit) | ((significandBits << 1) & ~highestBit), qd1.Exponent + qd2Exponent);
            }
            else
            {
                qd2Exponent = qd2.Exponent + 64;
                result = new Quaduple(((qd1.SignificandBits ^ qd2.SignificandBits) & highestBit) | (significandBits & ~highestBit), qd1.Exponent + qd2Exponent);
            }
        ...
        /// </summary>        
        public static bool operator ==(Quaduple qd1, Quaduple qd2)
        {
        ...
        /// <returns></returns>
        public static Quaduple Parse(string number)
        {
            ...
            if (right != null)
            {
                ...
                result += fractional * (Quaduple.Pow(new Quaduple(10L, 0), -right.Length));
            }
        ...
        /// <returns></returns>
        public static Quaduple Truncate(Quaduple value)
        {
            ...
            else
            {
                ...
                return new Quaduple((value.SignificandBits >> (int)(-value.Exponent)) << (int)(-value.Exponent), value.Exponent);
            }
        }
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static Quaduple Fraction(Quaduple value)
        {
            ...
            else
            {
                ...
                return new Quaduple((~highestBit & (bits << shift)) | (highestBit & value.SignificandBits), value.Exponent - shift);
            }
        }
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static double Log2(Quaduple value)
        {
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static double Log(Quaduple value)
        {
            if (value.SignificandBits >= highestBit) return double.NaN;
            if (value.Exponent <= notANumberExponent) return specialDoubleLogTable[(int)(value.Exponent - zeroExponent)];
        ...
        public static Quaduple Pow(Quaduple value, double exponent)
        {
            ...
            double resultSignificand = Math.Pow((double)new Quaduple(value.SignificandBits, -63), exponent);
            ...
            Quaduple result = (Quaduple)resultSignificand;
        ...
        ///
        /// Возврат:
        ///     Число e, возведенное в степень d. Если значение параметра d равно System.Double.NaN
        ///     или System.Double.PositiveInfinity, возвращается это значение. Если значение
        ///     параметра d равно System.Double.NegativeInfinity, возвращается значение 0.
        /// </summary>
        /// <param name="exponent"></param>
        /// <returns></returns>
        public static Quaduple Exp(double exponent)
        {
            return Pow(E, exponent);
        }

        public static Quaduple Max(Quaduple qd1, Quaduple qd2)
        {
            if (qd1.Exponent == notANumberExponent) return NaN;
            else return qd1 > qd2 ? qd1 : qd2;
        }

        public static Quaduple Min(Quaduple qd1, Quaduple qd2)
        {
            if (qd1.Exponent == notANumberExponent) return NaN;
            else return qd1 < qd2 ? qd1 : qd2;
        }

        public static Quaduple Abs(Quaduple qd)
        {
            if (qd.Exponent == negativeInfinityExponent) return PositiveInfinity;
            else return new Quaduple(qd.SignificandBits & ~highestBit, qd.Exponent); //clear the sign bit
        }

        public static int Sign(Quaduple qd)
        {
        ...
        public static bool IsNaN(Quaduple quad)
        {
            return quad.Exponent == notANumberExponent;
        }

        public static bool IsInfinity(Quaduple quad)
        {
            return quad.Exponent == infinityExponent || quad.Exponent == negativeInfinityExponent;
        }

        public static bool IsPositiveInfinity(Quaduple quad)
        {
            return quad.Exponent == infinityExponent;
        }

        public static bool IsNegativeInfinity(Quaduple quad)
        {
        ...
        #region Casts
        public static explicit operator Quaduple(System.Numerics.BigInteger value)
        {
            ...
            if (value <= ulong.MaxValue) //easy
            {
                ...
                int shift = nlz(bits);
                return new Quaduple((bits << shift) & ~highestBit | (positive ? 0 : highestBit), -shift);
            }
            else //can only keep some of the bits
            {
                ...
                if (bytes[bytes.Length - 1] == 0) //appended when the MSB is set to differentiate from negative values
                    return new Quaduple((positive ? 0 : highestBit) | (~highestBit & ((ulong)bytes[bytes.Length - 2] << 56 | (ulong)bytes[bytes.Length - 3] << 48 | (ulong)bytes[bytes.Length - 4] << 40 | (ulong)bytes[bytes.Length - 5] << 32 | (ulong)bytes[bytes.Length - 6] << 24 | (ulong)bytes[bytes.Length - 7] << 16 | (ulong)bytes[bytes.Length - 8] << 8 | (ulong)bytes[bytes.Length - 9])), (bytes.Length - 9) * 8);
                else //shift bits up
                {
                    ...
                    return new Quaduple((positive ? 0 : highestBit) | (~highestBit & bits), (bytes.Length - 8) * 8 - shift);
                }
            }
        }

        public static explicit operator System.Numerics.BigInteger(Quaduple value)
        {
            if (value.Exponent == negativeInfinityExponent
        ...
        public static explicit operator ulong(Quaduple value)
        {
            if (value.Exponent == negativeInfinityExponent
                || value.Exponent == infinityExponent)
        ...
        public static explicit operator long(Quaduple value)
        {
            if (value.Exponent == negativeInfinityExponent
                || value.Exponent == infinityExponent)
        ...
        public static unsafe explicit operator double(Quaduple value)
        {
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static explicit operator Quaduple(ulong value)
        {
            if (value == 0) return Zero;
            int firstSetPosition = nlz(value);
            return new Quaduple((value << firstSetPosition) & ~highestBit, -firstSetPosition);
        }

        public static implicit operator Quaduple(long value)
        {
            return new Quaduple(value, 0);
        }

        public static unsafe implicit operator Quaduple(double value)
        {
            // Translate the double into sign, exponent and mantissa.
            //long bits = BitConverter.DoubleToInt64Bits(value); // doing an unsafe pointer-conversion to get the bits is faster
            ulong bits = *((ulong*)&value);
            ...
            return new Quaduple((highestBit & bits) | mantissa, exponent);
        }
        ...
        {
            return ToString(QuadupleStringFormat.ScientificApproximate);
        }
        ...
        /// </remarks>
        public string ToString(QuadupleStringFormat format)
        {
            if (Exponent <= notANumberExponent) return specialStringTable[(int)(Exponent - zeroExponent)];

            switch (format)
            {
                case QuadupleStringFormat.HexExponential:
                ...
                case QuadupleStringFormat.ScientificApproximate:
                    ...
                    double dVal = (double)new Quaduple(SignificandBits, -61);
                ...
                case QuadupleStringFormat.ScientificExact:
        ...
        /// </summary>
        /// <returns></returns>
        private static string IntegerString(Quaduple quad, int digits)
        {
        ...
        /// <returns></returns>
        public int CompareTo(Quaduple other)
        {
